...
if (!isset($called_by_script_server)) {
	...
	print call_user_func_array('ss_lic_flexlm_license_svr_stats', $_SERVER['argv']);
}

function ss_lic_flexlm_license_svr_stats($cmd = 'index', $arg1 = '', $arg2 = '') {
	if ($cmd == 'index') {
		...
		return ss_lic_flexlm_svr_stats_getvalue($index, $arg);
	}
}

function ss_lic_flexlm_svr_stats_getvalue($server, $column) {
	$return_arr = array();

	if ($column == 'avgTime') $column = 'avg_time';
	elseif ($column == 'maxTime') $column = 'max_time';
	elseif ($column == 'lastTime') $column = 'cur_time';
...
function ss_lic_flexlm_svr_stats_getnames() {
	$return_arr = array();
...
function ss_lic_flexlm_svr_stats_getinfo($info_requested) {
	$return_arr = array();

	if ($info_requested == 'serverName') {
		$arr = db_fetch_assoc('SELECT
			service_id AS qry_index,
			server_name AS qry_value
			FROM lic_services
			ORDER BY qry_index');
	} elseif ($info_requested == 'serverID') {
		$arr = db_fetch_assoc('SELECT
			service_id AS qry_index,
			service_id AS qry_value
			FROM lic_services
			ORDER BY qry_index');
	}
