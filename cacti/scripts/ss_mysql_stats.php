...
function ss_mysql_stats($section = "", $host = "", $user = "", $passwd = "", $status_var = "") {
	/* collect [show status], [show variables], [slave status], [show ENGINE innodb status]
	 * into
	...
	if ($connection) {
		...
		if ($section == 'innodb') {
			$result_var      = @mysqli_query($connection, "show ENGINE innodb status");
		...
		if (is_resource($connection)) mysqli_close($connection);
		return "U";
	}
