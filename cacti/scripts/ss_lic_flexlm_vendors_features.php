...
if (!isset($called_by_script_server)) {
	...
	print call_user_func_array('ss_lic_get_vendors_features', $_SERVER['argv']);
}

function ss_lic_get_vendors_features($lic_server_id = 0, $cmd = 'index', $arg1 = '', $arg2 = '') {
	if ($cmd == 'index') {
		...
		return ss_lic_vendors_features($lic_server_id, $index, $arg);
	}
}

function ss_lic_vendors_features($lic_server_id, $featurename, $column) {
	$return_arr = array();
	...
	if ($column == 'inuse') {
		$arr = db_fetch_cell_prepared("SELECT
			$column_name
			FROM $table
			WHERE CONCAT(service_id, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature_name) = ?",
			array($featurename));
		...
		$arr = db_fetch_cell_prepared("SELECT COUNT(*)
			FROM $table
			WHERE action = ?
			AND CONCAT(portatserver, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature) = ?
			AND datetime BETWEEN ? AND ?",
			array($column_name, $featurename, $earlier_time, $current_time));
	}
...
function ss_lic_vendors_features_getnames($lic_server_id) {
	$return_arr = array();

	$arr = db_fetch_assoc_prepared('SELECT DISTINCT feature_name, vendor_daemon
		FROM lic_services_feature_use
		WHERE service_id = ?
		ORDER BY feature_name',
		array($lic_server_id));

	$arr = db_fetch_assoc_prepared("SELECT CONCAT(service_id, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature_name) AS feature_name
...
function ss_lic_vendors_features_getinfo($lic_server_id, $info_requested) {
	$return_arr = array();

	if ($info_requested == 'feature') {
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(service_id, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature_name) AS qry_index,
			CONCAT(vendor_daemon, '-', feature_name) AS qry_value
		...
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(lsfu.service_id, '=', REPLACE(lsfd.vendor_daemon, ' ', '_'), '=', lsfu.feature_name) AS qry_index,
		...
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(lsfd.service_id, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature_name) AS qry_index,
			CONCAT(lsfd.service_id, '=', REPLACE(vendor_daemon, ' ', '_'), '=', feature_name) AS qry_value
			FROM lic_services_feature_details AS lsfd
			WHERE service_id = ?
			ORDER BY feature_name",
			array($lic_server_id));
