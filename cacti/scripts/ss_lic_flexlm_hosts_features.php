...
if (!isset($called_by_script_server)) {
	...
	print call_user_func_array('ss_lic_get_hosts_features', $_SERVER['argv']);
}

function ss_lic_get_hosts_features($lic_server_id = 0, $cmd = 'index', $arg1 = '', $arg2 = '') {
	if ($cmd == 'index') {
		$return_arr = ss_lic_hosts_features_getnames($lic_server_id);

		for ($i=0;($i<cacti_sizeof($return_arr));$i++) {
			print $return_arr[$i] . "\n";
		}
	} elseif ($cmd == 'query') {
		$arr_index = ss_lic_hosts_features_getnames($lic_server_id);
		$arr = ss_lic_hosts_features_getinfo($lic_server_id, $arg1);
		...
		return ss_lic_hosts_features($lic_server_id, $index, $arg);
	}
}

function ss_lic_hosts_features($lic_server_id, $featurename, $column) {
	$return_arr = array();
...
function ss_lic_hosts_features_getnames($lic_server_id) {
	$return_arr = array();

	$arr = db_fetch_assoc_prepared("SELECT CONCAT(service_id, '=', hostname, '=', feature_name) AS feature_name
...
function ss_lic_hosts_features_getinfo($lic_server_id, $info_requested) {

	$return_arr = array();

	if ($info_requested == 'feature') {
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(service_id, '=', hostname, '=', feature_name) AS qry_index,
			CONCAT(hostname, '-', feature_name) AS qry_value
			FROM lic_services_feature_details
			WHERE service_id = ?
			AND hostname != ''
			ORDER BY feature_name",
		...
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(lsfu.service_id, '=', lsfd.hostname, '=', lsfu.feature_name) AS qry_index,
		...
		$arr = db_fetch_assoc_prepared("SELECT CONCAT(service_id, '=', hostname, '=', feature_name) AS qry_index,
			CONCAT(service_id, '=', hostname, '=', feature_name) AS qry_value
