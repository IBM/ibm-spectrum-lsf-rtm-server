...
/* rtm_get_daily_timespan   - returns start/end time for given date and timespan
...
function rtm_get_timespan(&$span, $curr_time, $timespan_given, $first_weekdayid,
	...
	switch ($timespan_given)  {
		case -1:
			$span['begin_now'] = time();
			$span['end_now']   = $span['begin_now'] + 86400;
			break;
		...
		case GT_DAY_SHIFT:
			...
			$span['begin_now'] = strtotime(date('Y-m-d', $curr_time) . ' ' . $read_user_cfg_func('day_shift_start'));
			$span['end_now'] = strtotime(date('Y-m-d', $curr_time) . ' ' . $read_user_cfg_func('day_shift_end'));
			break;
		case GT_THIS_DAY:
	...
	$span['current_value_date1'] = date($curr_time_fmt,$span['begin_now']);
	$span['current_value_date2'] = date($curr_time_fmt,$span['end_now']);
	$span['begin_now']           = $span['current_value_date1'];
	$span['end_now']             = $span['current_value_date2'];
}

/* rtm_month_shift	- check for shifting one or more months
 * @arg $shift_size	- requested shift amount
 * returns			- true, if month shifting required, else false
 */
function rtm_month_shift($shift_size) {
 ...
 */
function rtm_check_month_boundaries(&$span) {
	# check left boundary -----------------------------------------------
...
/* rtm_shift_right_boundary	- shift right boundary with end-of-month adjustment
 ...
 */
function rtm_shift_right_boundary(&$span, $direction, $shift_size) {
 ...
 */
function rtm_shift_time(&$span, $direction, $shift_size, $prefix = 'sess_ldst', $curr_time_fmt = 'Y-m-d') {
	...
	if (rtm_month_shift($shift_size) && rtm_check_month_boundaries($span)) {
		# shift left boundary
		$span['begin_now'] 	= strtotime($direction . $shift_size . ' ' . $span['current_value_date1']);
		# shifting right boundary is somewhat complicated
		$span['end_now'] 	= rtm_shift_right_boundary($span, $direction, $shift_size);
	} else {
