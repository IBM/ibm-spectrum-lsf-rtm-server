...
/* lic_get_daily_timespan   - returns start/end time for given date and timespan
...
function lic_get_daily_timespan(&$span, $curr_time, $timespan_given, $first_weekdayid) {
	...
	/*	case GT_LAST_HALF_HOUR:
		...
		case GT_DAY_SHIFT:
			...
			$span['begin_now'] = strtotime(date('Y-m-d', $curr_time) . ' ' . read_lic_config_option('day_shift_start'));
			$span['end_now'] = strtotime(date('Y-m-d', $curr_time) . ' ' . read_lic_config_option('day_shift_end'));
			break;
		case GT_THIS_DAY:
...
/* lic_month_shift	- check for shifting one or more months
 * @arg $shift_size	- requested shift amount
 * returns			- true, if month shifting required, else false
 */
function lic_month_shift($shift_size) {
 ...
 */
function lic_check_month_boundaries(&$span) {
	# check left boundary -----------------------------------------------
...
/* lic_shift_right_boundary	- shift right boundary with end-of-month adjustment
 ...
 */
function lic_shift_right_boundary(&$span, $direction, $shift_size) {
 ...
 */
function lic_shift_time(&$span, $direction, $shift_size, $prefix = 'sess_ldst') {
	...
	if (lic_month_shift($shift_size) && lic_check_month_boundaries($span)) {
		# shift left boundary
		$span['begin_now'] 	= strtotime($direction . $shift_size . ' ' . $span['current_value_date1']);
		# shifting right boundary is somewhat complicated
		$span['end_now'] 	= lic_shift_right_boundary($span, $direction, $shift_size);
	} else {
