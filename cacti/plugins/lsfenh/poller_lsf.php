	...
	if (cacti_sizeof($cluster)) {
		if ($cluster['disabled'] == 'on') {
			...
			if (grid_detect_and_correct_running_processes(0, 'LSFENH' . $clusterid, $frequency * 5)) {
				list($total_time, $jobs, $reservations) = grid_collect_jobs_remote($clusterid);
				...
				if ($total_time !== false) {
					...
					if ($jobs > 0) {
						cacti_log(sprintf('LSFENH Cluster STATS: Total:%4.2f, ClusterID:%s, Reservations:%s, Jobs:%s', $total_time, $clusterid, $reservations, $jobs), false, 'SYSTEM');
					}
				} else {
			...
			/* collect jobs */
			list($total_time, $jobs, $reservations) = grid_collect_jobs($clusterid);
