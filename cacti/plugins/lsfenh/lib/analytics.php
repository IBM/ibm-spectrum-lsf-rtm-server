...
function set_lsf_environment(&$cluster) {
	...
	$lsf_bindir = grid_get_lsfbindir($cluster);
...
function create_required_tables() {
	...
	db_execute("CREATE TABLE IF NOT EXISTS `grid_reservations` (
		`clusterid` int(10) unsigned NOT NULL DEFAULT 0,
		`rsvId` varchar(40) NOT NULL DEFAULT '',
		`rsvUnit` varchar(20) NOT NULL DEFAULT '',
		`rsvType` varchar(20) NOT NULL DEFAULT '',
		`noSusp` tinyint unsigned NOT NULL DEFAULT '0',
		`state` varchar(20) NOT NULL DEFAULT '',
		`user_or_group` varchar(128) NOT NULL DEFAULT '',
		`description` varchar(128) NOT NULL DEFAULT '',
		`queues` varchar(128) NOT NULL DEFAULT '',
		`creator` varchar(20) NOT NULL DEFAULT '',
		`totalSlots` int(10) unsigned NOT NULL DEFAULT 0,
		`numRsvHosts` int(10) unsigned NOT NULL DEFAULT 0,
		`numRsvJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`preScript` varchar(128) NOT NULL DEFAULT '',
		`postScript` varchar(128) NOT NULL DEFAULT '',
		`preTime` int(10) unsigned NOT NULL DEFAULT 0,
		`pendJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`runJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`suspJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`finishedJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`timeWindow` varchar(40) NOT NULL DEFAULT '',
		`nextInstance` varchar(40) NOT NULL DEFAULT '',
		`disabledDurations` varchar(512) NOT NULL DEFAULT '',
		`last_updated` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		PRIMARY KEY (`clusterid`,`rsvId`))
		ENGINE=InnoDB
		DEFAULT CHARSET=latin1
		COLLATE=latin1_swedish_ci
		ROW_FORMAT=DYNAMIC
		COMMENT='Holds Active Reservations'");

	db_execute("CREATE TABLE IF NOT EXISTS `grid_reservations_hosts` (
		`clusterid` int(10) unsigned NOT NULL DEFAULT 0,
		`rsvId` varchar(40) NOT NULL DEFAULT '',
		`host` varchar(64) NOT NULL DEFAULT '',
		`numJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`maxJobs` int(10) unsigned NOT NULL DEFAULT 0,
		`last_updated` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		PRIMARY KEY (`clusterid`,`rsvId`,`host`))
		ENGINE=InnoDB
		DEFAULT CHARSET=latin1
		COLLATE=latin1_swedish_ci
		ROW_FORMAT=DYNAMIC
		COMMENT='Holds Reservation Hosts'");

	db_execute("CREATE TABLE IF NOT EXISTS `grid_reservations_jobs` (
		`clusterid` int(10) unsigned NOT NULL DEFAULT 0,
		`rsvId` varchar(40) NOT NULL DEFAULT '',
		`jobid` bigint(20) unsigned NOT NULL DEFAULT 0,
		`indexid` int(10) unsigned NOT NULL DEFAULT 0,
		...
		`last_updated` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
		PRIMARY KEY (`clusterid`,`rsvId`,`jobid`,`indexid`))
		ENGINE=InnoDB
		DEFAULT CHARSET=latin1
		COLLATE=latin1_swedish_ci
		ROW_FORMAT=DYNAMIC
		COMMENT='Holds Reservation Jobs'");
}
...
function grid_collect_pend($clusterid) {
	...
	$reasons = shell_exec($lsf_bindir . '/bjobs -psum -p1 -uall 2>&1');
	grid_process_reasons($clusterid, $reasons, 'cluster', 0);

	// Get summary suspended reasons
	$suspend = shell_exec($lsf_bindir . '/bjobs -uall -s 2>&1');
...
function grid_collect_sla_reasons($sla, $clusterid) {
	...
	// Get summary pending reasons
	if ($remote != 'on') {
		$reasons = shell_exec($lsf_bindir . "/bjobs -psum -p1 -uall -sla $sla 2>&1");
...
function grid_update_job_groups($clusterid) {
	...
	$job_lines     = array();
	$slot_lines    = array();
	$bjgroup_jobs  = shell_exec($lsf_bindir . "/bjgroup -s 2>&1");
	$jsql = $ssql  = array();
	...
	if ($bjgroup_jobs != '') {
		...
		if (strpos($job_lines[0], 'No job group found') === false && strpos($job_lines[0], 'Failed in an LSF library') === false) {
			$bjgroup_slots = shell_exec($lsf_bindir . "/bjgroup -N 2>&1");
...
function grid_hostgroup_sla_definitions($clusterid) {
	...
	$bmgroup = shell_exec($lsf_bindir . "/bmgroup -w 2>&1");

	grid_process_bmgroup($clusterid, $bmgroup);

	$bresources = shell_exec($lsf_bindir . "/bresources -g 2>&1");
...
function grid_process_bresources($clusterid, $group) {
	...
	$bresources = shell_exec($lsf_bindir . "/bresources -g -l $group 2>&1 | grep 'HOSTS:'");
...
function grid_collect_shared_descriptions($clusterid) {
	...
	// Get summary pending reasons
	if ($remote != 'on') {
		$lsinfo = shell_exec($lsf_bindir . "/lsinfo -w 2>&1");
...
function grid_get_lsfbindir($cluster = []) {
	global $config;
	...
	/* try for the real path first */
	if (isset($clsuter['lsf_bindir']) && is_dir($cluster['lsf_envdir'])) {
		$bindir = str_replace('/conf', '', $cluster['lsf_envdir']) . '/10.1/linux3.10-glibc2.17-x86_64/bin';

		if (is_dir($bindir)) {
			return $bindir;
		} else {
			$bindir = str_replace('/conf', '', $cluster['lsf_envdir']) . '/10.1/linux2.6-glibc2.3-x86_64/bin';
	...
	/* got for the user setting in the database */
	$bindir = read_config_option('path_lsf_bindir');

	if ($bindir != '' && is_dir($bindir)) {
		return $bindir;
	} elseif (is_dir($config['base_path'] . '/plugins/lsfenh/bin')) {
		$bindir = $config['base_path'] . '/plugins/lsfenh/bin';

		return $bindir;
	} else {
		$location = shell_exec('which bjobs');

		if ($location != '') {
			$bindir = str_replace('/bjobs', '', $location);
	...
	if (!$logged) {
		cacti_log('FATAL: Unable to determine the LSF binary directory.  Go to Settings > RTM Plugins and set the LSF_BINDIR', false, 'LSFENH');
		$logged = true;
	}
...
function grid_collect_reservations($clusterid) {
	global $config;
	...
	/**
	 * Get advanced reservations using the wide format to get the full names.
	 * We will use this pass to get the hosts, users and then, we will make
	 * a call to get the reservation details by reservation name.
	 *
	 * We do this as the brsvs command has not json options and it's wide
	 * format is not compatible with long listing output.  The long listing
	 * output truncates:
	 *
	 * 1. The reservation id
	 * 2. Users
	 * 3. Hosts
	 *
	 */
	$reservations = shell_exec($lsf_bindir . "/brsvs -w 2>&1 | grep -v RSVID");

	// Catch the case of no reservations
	if (str_contains($reservations, 'No reservation found')) {
		return $rsvcnt;
	}

	// Catch API errors
	if (str_contains($reservations, 'Failed in an LSF library')) {
		return $rsvcnt;
	}

	// Explode the reservations into a list
	$reservations = explode("\n", $reservations);

	// Skip reservations as they are from a foreign cluster
	$skip_res = [];

	// Normalize the wide output reservations
	foreach($reservations as $index => $r) {
		$r = trim($r);

		$resdata = preg_split('/[\s]+/', $r);
		$entries = sizeof($resdata);

		switch ($entries) {
			case 1:
				// User entry only
				$users[$curres][] = $resdata[0];
				break;
			case 3:
				// Host entry only
				$hosts[$curres][] = [
					'hostname' => explode(':', $resdata[0])[0],
					'resslots' => explode(':', $resdata[0])[1],
					'totslots' => $resdata[2]
				];
			...
			case 4:
				...
				$hosts[$curres][] = [
					'hostname' => explode(':', $resdata[1])[0],
					'resslots' => explode(':', $resdata[1])[1],
					'totslots' => $resdata[3]
				];
			...
			case 10:
			case 11:
				// Start Reservation entry
				$curres = $resdata[0];

				if (str_contains($curres, '@')) {
					$skip_res[$curres] = $curres;
				}

				$rsvs[$curres]['name']      = $curres;
				$rsvs[$curres]['type']      = $resdata[1];
				$rsvs[$curres]['resslots']  = $resdata[3];
				$rsvs[$curres]['totslots']  = $resdata[5];

				if ($entries == 11) {
					$rsvs[$curres]['timestart'] = $resdata[9];
					$rsvs[$curres]['timeend']   = $resdata[10];
				} else {
					/* split the window into a date range */
					[$start, $end] = explode('-', $resdata[9]);

					$rsvs[$curres]['timestart'] = substr($start, 0, 4) . '-' . substr($start, 5, 2) . '-' . substr($start, 8, 2) . ' ' . substr($start, 11, 2) . ':' . substr($start, 14, 2);
					$rsvs[$curres]['timeend']   = substr($end, 0, 4) . '-' . substr($end, 5, 2) . '-' . substr($end, 8, 2) . ' ' . substr($end, 11, 2) . ':' . substr($end, 14, 2);
				}

				$hosts[$curres][] = [
					'hostname' => explode(':', $resdata[6])[0],
					'resslots' => explode(':', $resdata[6])[1],
					'totslots' => $resdata[8]
				];

				$users[$curres][] = $resdata[2];

				break;
			default:
				// Unknown pattern so log
				cacti_log("WARNING: The reservation line $r does not match a pattern with $entries entries.", false, 'LSFENH');
	...
	$patterns = [
		...
		'Creator:'            => 'creator',
		'Reservation Type:'   => 'rsvType',
		'Resource Unit:'      => 'rsvUnit',
		'PEND Jobs:'          => 'pendJobs',
		'RUN  Jobs:'          => 'runJobs',
		'SUSP Jobs:'          => 'suspJobs',
		'FINISHED Jobs:'      => 'finishedJobs',
		'Pre-AR Script:'      => 'preScript',
		'Post-AR Script:'  	  => 'postScript',
		'Time Window:'     	  => 'timeWindow',
		'Start action:'       => 'noSusp',
		'Next Active Period:' => 'nextInstance',
		'Queues that can use AR hosts before AR starts:' => 'queues',
	];
	...
	$sql_prefix = 'INSERT INTO `grid_reservations` (`' . implode('`,`', $columns) . '`) VALUES ';

	$sql_suffix = 'ON DUPLICATE KEY UPDATE ';

	foreach($columns as $i => $c) {
		if ($c != 'last_updated') {
			$sql_suffix .= ($i > 0 ? ',':'') . "`$c` = VALUES(`$c`)";
		} else {
			$sql_suffix .= ($i > 0 ? ',':'') . "`$c` = NOW()";
		}
	}
	...
	foreach($rsvs as $index => $resdata) {
		$name = $resdata['name'];

		// Remove multi-cluster shared reservations
		if (array_key_exists($name, $skip_res)) {
			unset($rsvs[$index]);
			unset($hosts[$name]);
			unset($users[$name]);
		...
		$data = shell_exec($lsf_bindir . "/brsvs -l $name 2>&1 | grep -v RSVID");

		// Catch the case an invalid reservation
		if (str_contains($data, 'Reservation ID is not valid')) {
			cacti_log("WARNING: Unable to find RSVID $name", false, 'LSFENH');
		...
		foreach($lines as $l) {
			...
			if ($inres) {
				...
				// Initialize all columns for database inserts
				foreach($columns as $string => $column) {
					switch($column) {
						case 'clusterid':
							break;
						case 'pendJobs':
			...
			if (str_starts_with($l, 'Reservation Status:')) {
				$inhost = false;
				$inresf = false;
			} elseif (str_starts_with($l, 'Next Active Period:')) {
				$inraw  = true;

				continue;
			} elseif (str_starts_with($l, 'Start action:')) {
				$inraw  = true;
			...
			foreach($patterns as $string => $column) {
				if (str_starts_with($l, $string)) {
					$l = trim(str_replace($string, '', $l));
		...
		/* end processing reservation data */
		...
		$params[] = $noSusp ?? '';
		$params[] = $state;
		$params[] = trim(implode(', ', $users[$name]), ',');
		$params[] = $description;
		$params[] = $queues ?? '';
		$params[] = $creator;
		$params[] = $rsvs[$name]['totslots'];
		$params[] = sizeof($hosts[$name]);
		$params[] = $rsvs[$name]['resslots'];
		$params[] = $preScript ?? '';
		$params[] = $postScript ?? '';
		$params[] = $preTime ?? 0;
		$params[] = cacti_sizeof($pendJobs);
		$params[] = cacti_sizeof($runJobs);
		$params[] = cacti_sizeof($suspJobs);
		$params[] = cacti_sizeof($finishedJobs);
		$params[] = $timeWindow;
		$params[] = $nextInstance;
		...
		// Handle Jobs
		foreach($pendJobs as $j) {
			$jsql[]    = '(?, ?, ?, ?, ?)';
			$jparams[] = $clusterid;
			$jparams[] = $name;
			$jparams[] = explode('[', $j)[0];
			$jparams[] = str_contains($j, '[') ? trim(explode('[', $j)[1], ']') : 0;
			$jparams[] = 'PEND';
		}

		foreach($runJobs as $j) {
			$jsql[]    = '(?, ?, ?, ?, ?)';
			$jparams[] = $clusterid;
			$jparams[] = $name;
			$jparams[] = explode('[', $j)[0];
		...
		if (cacti_sizeof($jsql)) {
			db_execute_prepared('INSERT INTO grid_reservations_jobs
				(clusterid, rsvId, jobid, indexid, status)
				VALUES ' . implode(', ', $jsql) . '
				ON DUPLICATE KEY UPDATE `status` = VALUES(`status`), last_updated = NOW()', $jparams);
		}
	}
	...
	//$hosts[$curres][index]['hostname'] = explode(':', $resdata[1])[0];
	//$hosts[$curres][index]['resslots'] = explode(':', $resdata[1])[1];
	//$hosts[$curres][index]['totslots'] = $resdata[3];

	if (cacti_sizeof($hosts)) {
		foreach($hosts as $name => $reshosts) {
			foreach($reshosts as $host) {
				$hsql[]    = '(?, ?, ?, ?, ?)';

				$hparams[] = $clusterid;
				$hparams[] = $name;
				$hparams[] = $host['hostname'];
				$hparams[] = $host['resslots'];
				$hparams[] = $host['totslots'];
			}
		}

		db_execute_prepared('INSERT INTO grid_reservations_hosts
			(clusterid, rsvId, host, numJobs, maxJobs)
			VALUES ' . implode(', ', $hsql) . '
			ON DUPLICATE KEY UPDATE numJobs = VALUES(numJobs), maxJobs = VALUES(maxJobs), last_updated = NOW()', $hparams);
	}
	...
	if (cacti_sizeof($users)) {
		...
		foreach($users as $name => $udata) {
			foreach($udata as $user) {
				$usql[]    = '(?, ?, ?, ?)';

				$uparams[] = $clusterid;
				$uparams[] = $name;
				$uparams[] = $rsvs[$name]['type'];
				$uparams[] = $user;
			}
		}

		db_execute_prepared('INSERT INTO grid_reservations_users
			(clusterid, rsvId, type, user_or_group)
			VALUES ' . implode(', ', $usql) . '
			ON DUPLICATE KEY UPDATE last_updated = NOW()', $uparams);
	}

	if (sizeof($sql)) {
		//cacti_log(vsprintf(str_replace('?', '"%s"', $sql_prefix . implode(', ', $sql) . $sql_suffix), $params));
		db_execute_prepared($sql_prefix . implode(', ', $sql) . $sql_suffix, $params);
	}

	// Remove old records
	db_execute_prepared('DELETE FROM grid_reservations WHERE clusterid = ? AND last_updated < ?', [$clusterid, $update]);
	db_execute_prepared('DELETE FROM grid_reservations_hosts WHERE clusterid = ? AND last_updated < ?', [$clusterid, $update]);
	db_execute_prepared('DELETE FROM grid_reservations_users WHERE clusterid = ? AND last_updated < ?', [$clusterid, $update]);
	db_execute_prepared('DELETE FROM grid_reservations_jobs WHERE clusterid = ? AND last_updated < ?', [$clusterid, $update]);
...
function grid_collect_jobs_remote($clusterid) {
	...
	if (!is_grid_process_running('0', 'OPTIMIZE')) {
		...
		$reservations = grid_collect_reservations($clusterid);
	...
	return array($end - $start, $total_jobs, $reservations);
}
...
function grid_collect_jobs($clusterid) {
	...
	$jobs = shell_exec($lsf_bindir . "/bjobs -uall -o 'jobid jobindex user project app stat queue sla:60 first_host slots min_req_proc max_req_proc cpu_used max_mem avg_mem mem run_time submit_time start_time finish_time effective_resreq combined_resreq' -json 2>/dev/null");
...
function grid_planner_jobs($clusterid) {
	...
	$jobs = file(shell_exec($lsf_bindir . "/bjobs -uall -UF -plan 2>/dev/null"));
