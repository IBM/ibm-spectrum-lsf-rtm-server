...
if ($clusterid !== false && $clusterid > 0) {
	$cluster = db_fetch_row_prepared('SELECT clustername, lsf_envdir, lsf_clustername, disabled, remote
	...
	if (cacti_sizeof($cluster)) {
		if ($cluster['disabled'] == 'on') {
			...
			print "WARNING: Attempting to collect jobs from a remotely collected clusterid $clusterid." . PHP_EOL;
		} elseif (grid_detect_and_correct_running_processes(0, 'LSFENHPEND' . $clusterid, $frequency * 5)) {
			set_config_option('grid_jobs_pend_' . $clusterid, $now);

			if (set_lsf_environment($cluster)) {
				/* collect jobs */
				$total_time = grid_collect_pend($clusterid);
			} else {
				$total_time = false;
			}

			/* remove the process entry */
			grid_remove_process_entry(0, 'LSFENHPEND' . $clusterid);
...
function display_version() {
	print 'RTM Jobs Pend Reasons Collector for Analytics ' . read_config_option('grid_version') . PHP_EOL;
...
function display_help() {
	...
	print 'usage: poller_pend.php --clusterid=N [-f|--force] [-d|--debug]' . PHP_EOL;
}
