<?php
// $Id: c39f3d778a2d75d3d49f0fd3870a36e30ba01f03 $
/*
 +-------------------------------------------------------------------------+
...
function form_actions() {
	...
	load_current_session_value('type', 'sess_gmh_type', '0');
	load_current_session_value('integrated', 'sess_gmh_integrated', '-1');
...
function grid_get_host_records(&$sql_where, &$sql_join, $apply_limits = true, $rows, &$sql_params) {
	...
	if (get_request_var('integrated') != '2' || get_request_var('type') != 0) {
		/* clusterid sql where */
		...
		if (!strlen($sql_join)) {
			$sql_join .= ' RIGHT JOIN grid_hostinfo ON grid_clusters.clusterid = grid_hostinfo.clusterid)';
		}
...
function grid_manage_hosts() {
	...
	$display_text = array(
		'clustername' => array(
			'display' => __('Cluster Name'),
			'sort' => 'ASC'
		),
		'host'        => array(
			'display' => __('Host Name'),
			'sort'    => 'ASC'
		),
		'hostType'    => array(
			'display' => __('Host Type'),
			'sort'    => 'DESC'
		),
		'hostModel'   => array(
			'display' => __('Host Model'),
			'sort'    => 'DESC'
		),
		'isServer'    => array(
			'display' => __('Host License'),
			'sort'    => 'DESC'
		),
		'first_seen'  => array(
			'display' => __('First Seen'),
			'align'   => 'right',
			'sort'    => 'DESC'
		),
		'last_seen'   => array(
			'display' => __('Last Seen'),
			'align'   => 'right',
			'sort'    => 'ASC'
		)
	);
	...
	if (cacti_sizeof($hosts)) {
		if (strlen(get_request_var('filter')) >0){
			foreach ($hosts as $host) {
				...
				form_selectable_cell($display_value, $id, '', 'right');
	...
	$(function() {
		if ($('#type').val() != 0) {
			$('#integrated').val('-1');
			if ($('#integrated').selectmenu('widget')) {
				$('#integrated').prop('disabled', true);
				$('#integrated').selectmenu('refresh').selectmenu('disable');
			} else {
				$('#integrated').prop('disabled', true);
			}
		}
